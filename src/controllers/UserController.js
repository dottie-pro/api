...
class UserController {
  ...
  uploadLogo = async (req, res) => {
    try {
      const { userId = null } = req.currentUser;

      const { name, size, key, location: url = "" } = req.file;

      const existsLogo = await LogoModel.findOne({ userId });
      ...
      if (existsLogo) {
        ...
        await deleteObjectFromS3(existsLogo.key);
      } else {
        logo = await LogoModel.create({
          name,
          size,
          key,
          url,
      ...
      return res.status(201).json({ logo: logo, success: true });
    } catch (error) {
      console.log(error);
  ...
  getLogo = async (req, res) => {
    try {
      const { userId } = req.currentUser;
      const logo = await LogoModel.findOne({ userId: userId });
  ...
  deleteLogo = async (req, res) => {
    try {
      ...
      await deleteObjectFromS3(deletedLogo.key);
      res.status(200).json({ deletedLogo, success: true });
    } catch (error) {
      res.status(500).json({ error, success: false });
    }
  };
